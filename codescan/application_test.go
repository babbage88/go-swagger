...
func loadPetstorePkgsCtx(t testing.TB) *scanCtx {
	...
	sctx, err := newScanCtx(&Options{
		Packages: []string{"github.com/babbage88/go-swagger/fixtures/goparsing/petstore/..."},
	})
...
func loadClassificationPkgsCtx(t testing.TB, extra ...string) *scanCtx {
	...
	sctx, err := newScanCtx(&Options{
		Packages: append([]string{
			"github.com/babbage88/go-swagger/fixtures/goparsing/classification",
			"github.com/babbage88/go-swagger/fixtures/goparsing/classification/models",
			"github.com/babbage88/go-swagger/fixtures/goparsing/classification/operations",
		}, extra...),
	})
	require.NoError(t, err)
...
func TestAppScanner_Definitions(t *testing.T) {
	doc, err := Run(&Options{
		Packages:   []string{"github.com/babbage88/go-swagger/fixtures/goparsing/bookings/..."},
		ScanModels: true,
	})
	require.NoError(t, err)
	if assert.NotNil(t, doc) {
