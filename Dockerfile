...
RUN mkdir -p bin &&\
  LDFLAGS="$LDFLAGS -X github.com/babbage88/go-swagger/cmd/swagger/commands.Commit=${commit_hash}" &&\
  LDFLAGS="$LDFLAGS -X github.com/babbage88/go-swagger/cmd/swagger/commands.Version=${tag_name}" &&\
